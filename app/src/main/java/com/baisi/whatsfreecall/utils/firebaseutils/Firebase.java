...
public class Firebase extends AbstractFirebase {
    ...
    public void _logEvent(String key, String name1, String value1) {
        FirebaseAnalytics firebaseAnalytics = instance.getFirebaseAnalytics();
        Bundle bundle = new Bundle();
            ...
            public void onComplete(@NonNull Task<String> task) {
                if (appInstanceIdListener != null) {
                    try {
                        ...
                        appInstanceIdListener.setAppInstanceId("RunTimeExtcutionExceptionTimeoutException");
                    }
     ...
     *
     * @param envent  事件名称
     * @param map  属性集合 对应key 的属性集合
     */
    public void logEvent(String envent, Map<String, String> map) {
        Set<String> set = map.keySet();
        Iterator<String> it = set.iterator();
        Bundle bundle = new Bundle();
        while (it.hasNext()) {
            String mKey = it.next();
            bundle.putString(mKey, map.get(mKey));
        }
        FirebaseAnalytics firebaseAnalytics = instance.getFirebaseAnalytics();
        System.out.println("Firebase.logEvent");
        firebaseAnalytics.logEvent(envent, bundle);
    }
    /**
     *
     * @param envent  事件名称
     * @param bundle  属性bundle
     */
    public void logEvents(String envent, Bundle bundle) {
        FirebaseAnalytics firebaseAnalytics = instance.getFirebaseAnalytics();
        System.out.println("Firebase.logEvents");
        firebaseAnalytics.logEvent(envent, bundle);
    }
}